...
import { DB44_RECORDS, BLEND_COLORS, parsePPRatio, isComposite } from '../../lib/database44';
import { Beaker, TrendingUp, Layers, Info, ToggleLeft, ToggleRight } from 'lucide-react';

// ── Blending Performance Bar Chart ─────────────────────────────────────────
function BlendPerformanceChart() {
  const [baselineToggle, setBaselineToggle] = useState(false);

  const baseMean = useMemo(() => {
    const base = DB44_RECORDS.filter(r => r.blend === 'Non');
    return base.reduce((s, r) => s + r.co2Uptake, 0) / base.length;
  }, []);
...
function BlendRatioCorrelation() {
  const ratioData = useMemo(() => {
    return DB44_RECORDS
      .filter(r => parsePPRatio(r.blend) !== null)
      .map(r => ({ ppRatio: parsePPRatio(r.blend), co2: r.co2Uptake, blend: r.blend, biomass: r.biomass }));
  }, []);

  const baselineMean = useMemo(() => {
    const base = DB44_RECORDS.filter(r => r.blend === 'Non' && r.activator === 'Non');
...
// ── Main export ─────────────────────────────────────────────────────────────
export default function BlendingAnalysis() {
  return (
    <div className="space-y-5">
      ...
      <BlendPerformanceChart />
      <BlendRatioCorrelation />
    </div>
  );
}