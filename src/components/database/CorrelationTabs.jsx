...
import { DB44_RECORDS, BIOMASS_COLORS, ADSORPTION_TEMP_LIST, BIOMASS_LIST, ACTIVATOR_LIST } from '../../lib/database44';
import {
...
const TABS = ['Isotherm Plots', 'Multidimensional', 'Feature Effects', 'Correlation Heatmap', 'Anomaly Detection'];

// ── Tab A: Isotherm Plot ──────────────────────────────────────────────────────
function IsothermTab() {
  const [adsorpTemp, setAdsorpTemp] = useState(25);
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          {ADSORPTION_TEMP_LIST.map(t => (
            <button
              key={t}
              onClick={() => setAdsorpTemp(t)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                adsorpTemp === t ? 'gradient-green text-white' : 'bg-muted hover:bg-muted/80 text-foreground'
              }`}
            >{t}°C</button>
          ))}
        </div>
      ...
      <p className="text-xs text-muted-foreground">
        CO₂ Uptake (mmol/g) vs. Adsorption Pressure (atm) — each line is a unique experimental entry at {adsorpTemp}°C
      </p>
      <ResponsiveContainer width="100%" height={320}>
        <LineChart margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
          ...
          <XAxis dataKey="pressure" type="number" domain={[0, 1.1]} tickCount={6}
            ...
            tick={{ fontSize: 11 }} unit=" atm" />
          <YAxis label={{ value: 'CO₂ Uptake (mmol/g)', angle: -90, position: 'insideLeft', fontSize: 11 }}
            tick={{ fontSize: 11 }} unit=" mmol/g" />
          <Tooltip formatter={(v, n) => [`${Number(v).toFixed(3)} mmol/g`, n]} />
          <Legend wrapperStyle={{ fontSize: 10, paddingTop: 8 }} />
          {Object.entries(seriesMap).map(([key, data], i) => (
            <Line
              key={key} name={key} data={data}
              dataKey="co2Uptake" type="monotone"
...
function MultiDimTab() {
  ...
  const seriesByBiomass = useMemo(() => {
    const out = {};
    BIOMASS_LIST.forEach(b => {
      out[b] = DB44_RECORDS.filter(r => r.biomass === b).map(r => ({
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-xs text-muted-foreground">
        Color = Biomass Species · Bubble size ∝ {zAxis === 'co2Uptake' ? 'CO₂ Uptake' : zAxis}
      </p>
      <ResponsiveContainer width="100%" height={340}>
        <ScatterChart margin={{ top: 10, right: 20, left: 0, bottom: 20 }}>
          ...
          {BIOMASS_LIST.map(b => (
            <Scatter key={b} name={b.split(' ')[0] + (b.includes('Coffee') ? ' (Coffee)' : b.includes('Pine') ? ' (Pine)' : ' (Corn)')}
              data={seriesByBiomass[b]} fill={BIOMASS_COLORS[b]} fillOpacity={0.75} />
          ))}
        </ScatterChart>
      </ResponsiveContainer>
    </div>
...
// ── Tab C: Feature Effects Box-style Bar ──────────────────────────────────────
function FeatureEffectsTab() {
  ...
  const chartData = useMemo(() => {
    ...
    return Object.entries(groups).map(([name, vals]) => {
      ...
      return { name: name.replace(' ground-based', '').replace(' sawdust powders', ''), mean: +mean.toFixed(3), min: +min.toFixed(3), max: +max.toFixed(3), q1: +q1.toFixed(3), q3: +q3.toFixed(3), count: vals.length };
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {[{ key: 'activator', label: 'Activator' }, { key: 'activationType', label: 'Activation Type' }, { key: 'biomass', label: 'Biomass Species' }].map(opt => (
          <button key={opt.key} onClick={() => setFeature(opt.key)}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${feature === opt.key ? 'gradient-blue text-white' : 'bg-muted hover:bg-muted/80 text-foreground'}`}>
            {opt.label}
          </button>
        ))}
      </div>
      ...
      <ResponsiveContainer width="100%" height={320}>
        <BarChart data={chartData} margin={{ top: 16, right: 20, left: 0, bottom: 10 }}>
          ...
          <YAxis unit=" mmol/g" tick={{ fontSize: 11 }} />
          <Tooltip
            formatter={(v, n) => [`${v} mmol/g`, n]}
          ...
          <Bar dataKey="mean" name="Mean CO₂ Uptake" radius={[6, 6, 0, 0]}>
            ...
            <LabelList dataKey="mean" position="top" style={{ fontSize: 10, fontWeight: 700 }} formatter={v => `${v}`} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
      {/* Stats table */}
    ...
    <div className="glass-card rounded-2xl p-5 border border-border">
      <div className="mb-5">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          Interactive multidimensional exploration — all values derived directly from 44Database.xlsx
        </p>
      </div>
      ...
      {activeTab === 0 && <IsothermTab />}
      {activeTab === 1 && <MultiDimTab />}
      {activeTab === 2 && <FeatureEffectsTab />}
      {activeTab === 3 && <CorrelationHeatmap />}
      {activeTab === 4 && <AnomalyDetectionTab />}
    </div>
  );
}