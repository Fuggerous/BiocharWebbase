...
function MultiDimTab({ records }) {
  ...
  // 2D bubble: series grouped by biomass
  ...
  // 3D: flat array with colour per biomass
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-end gap-4">
        {[
          { label: view === '3d' ? 'X-Axis (Red)'   : 'X-Axis',          val: xAxis, set: setXAxis },
          { label: view === '3d' ? 'Y-Axis (Green)'  : 'Y-Axis',          val: yAxis, set: setYAxis },
          { label: view === '3d' ? 'Z-Axis (Blue)'   : 'Bubble Size (Z)', val: zAxis, set: setZAxis },
        ].map(({ label, val, set }) => (
        ...
        {/* 2D / 3D toggle */}
        <div className="ml-auto flex items-center gap-1 p-1 bg-muted rounded-xl border border-border self-end">
          {[{ key: '2d', label: '2D Bubble' }, { key: '3d', label: '3D Scatter' }].map(v => (
            <button key={v.key} onClick={() => setView(v.key)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                view === v.key ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'
              }`}>
              {v.label}
            </button>
          ))}
      ...
      {view === '3d' && (
        <>
          <p className="text-xs text-muted-foreground">
            <span className="font-semibold text-foreground">{points3d.length}</span> records · Color = Biomass
          </p>
          <ScatterPlot3D
