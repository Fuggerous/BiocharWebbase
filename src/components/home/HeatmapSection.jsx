...
export default function HeatmapSection() {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-amber-50 via-white to-orange-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 relative overflow-hidden">
      <div className="absolute inset-0 bg-grid-pattern opacity-20" />
      <div className="absolute top-1/2 right-0 w-96 h-96 bg-amber-100/40 rounded-full blur-3xl -translate-y-1/2" />
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-100 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-800/50 mb-4">
            ...
            <span className="text-amber-700 text-sm font-medium">{t('heatmap.badge')}</span>
          </div>
        ...
        <motion.div
          ...
          className="glass-card rounded-3xl border border-amber-200 dark:border-amber-900/30 shadow-xl shadow-amber-50 dark:shadow-black/30 p-6 md:p-8"
        >
          {/* Hover info */}
          <div className="h-12 mb-4 flex items-center justify-center">
            {hovered ? (
              <motion.div
                ...
                className="flex items-center gap-3 px-5 py-2 rounded-xl border text-sm font-semibold shadow-sm"
                style={{
                  background: `${heatColor(norm(hovered.value))}18`,
                  borderColor: `${heatColor(norm(hovered.value))}50`,
                  color: 'hsl(var(--foreground))',
                }}
              >
                <span className="w-3 h-3 rounded-full flex-shrink-0" style={{ background: heatColor(norm(hovered.value)) }} />
                <span>
                  <span className="text-muted-foreground font-normal">{ACTIVATOR_LABELS[hovered.activator] ?? hovered.activator} · {hovered.temp}°C</span>
                  {'  →  '}
                  <span className="font-bold" style={{ color: heatColor(norm(hovered.value)) }}>{hovered.value.toFixed(2)} mmol/g</span>
                  {norm(hovered.value) >= 0.85 && <span className="ml-2 px-2 py-0.5 rounded-full bg-red-100 text-red-600 border border-red-200 text-xs">🔥 {t('heatmap.peakZone')}</span>}
                  {norm(hovered.value) >= 0.55 && norm(hovered.value) < 0.85 && <span className="ml-2 px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 border border-amber-200 text-xs">{t('heatmap.highPerformance')}</span>}
                  {norm(hovered.value) < 0.25 && <span className="ml-2 px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 border border-blue-200 text-xs">{t('heatmap.lowActivity')}</span>}
                </span>
              </motion.div>
            ) : (
          ...
          <div className="overflow-x-auto">
            <div style={{ minWidth: `${120 + temps.length * 80}px` }}>
              ...
              <div className="grid mb-2" style={{ gridTemplateColumns: `120px repeat(${temps.length}, 1fr)`, gap: '6px' }}>
                <div />
                {temps.map(t => (
                  <div key={t} className="text-center text-xs font-bold text-muted-foreground py-1">{t}°C</div>
                ))}
              </div>
              ...
              <div className="space-y-1.5">
                {activators.map(act => (
                  <div key={act} className="grid items-center" style={{ gridTemplateColumns: `120px repeat(${temps.length}, 1fr)`, gap: '6px' }}>
                    <div className="text-xs font-semibold text-muted-foreground text-right pr-3 py-1">
                    ...
                    {temps.map(t => {
                      ...
                      if (val == null) {
                        return (
                          <div key={t} className="rounded-xl flex items-center justify-center text-slate-400 text-xs" style={{ height: 52, background: 'rgba(100,116,139,0.06)', border: '1px dashed rgba(148,163,184,0.3)' }}>
                            —
                          </div>
                        );
                      }
                      ...
                      return (
                        <motion.div
                          key={t}
                          className={`relative rounded-xl cursor-pointer flex items-center justify-center font-space font-bold text-sm select-none ${textLight ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}
                          style={{
                            background: bg,
                            height: 52,
              ...
              <div className="mt-6 flex items-center justify-between gap-4">
                <div className="flex items-center gap-2 flex-1">
                  <span className="text-[10px] text-muted-foreground shrink-0">{t('heatmap.low')} ({globalMin.toFixed(1)})</span>
                  ...
                  <span className="text-[10px] text-muted-foreground shrink-0">{t('heatmap.peak')} ({globalMax.toFixed(1)}) mmol/g</span>
                </div>
              </div>
            </div>
          </div>
